...
class WXPayUtil {
    ...
    void pay ( String body, String fee ) {
        ...
        api = WXAPIFactory.createWXAPI ( context, Contant.WXPAY_ID );
        ...
        Runnable payRunnable = new Runnable ( ) {
            ...
            void run ( ) {
                ...
                if ( prePayResult.localRetCode != LocalRetCode.ERR_OK ) {
                    String msgString = String.format ( "生成预订单失败，错误代码%d,原因%s", prePayResult
                                                               .errCode, prePayResult.errMsg );
                    Message msg = handler.obtainMessage ( SDK_WX_ACCESSTOKEN_FAIL, msgString );
                    handler.sendMessage ( msg );
                    return;
                }
     ...
     */
    private
    void sendPayReq ( GetPrepayIdResult result ) {
    ...
    private GetAccessTokenResult getAccessToken()
    {
        ...
        if (AccessToken == null || AccessToken.length()<1 || second > 5400)
        {
            String url = String.format(
                    ...
                    tokenUrl, Contant.WXPAY_ID, Contant.WXPAY_SECRT);
    ...
    {
        String userName = application.readNickName ( );
    ...
    private String genProductArgs()
    {
        ...
        try
        {
            json.put("appid", Contant.WXPAY_ID);
            // traceId
            ...
            List<NameValuePair> signParams = new LinkedList<NameValuePair>();
            signParams.add(new BasicNameValuePair("appid", Contant.WXPAY_ID));
            signParams.add(new BasicNameValuePair("appkey", APP_KEY));
